...
class CustomPaginationActionsTable extends React.Component {
    ...
    fetchData = () => {
        axios
            .get('https://ytkqcmybo7.execute-api.eu-west-1.amazonaws.com/dev/getInstitutionData')
            .then(response => {
                this.setState({
                    data: response.data,
                    loading: false,
    ...
    render() {
        ...
        const { data, loading, error, rowsPerPage, page, searchQuery } = this.state;
        const emptyRows = rowsPerPage - Math.min(rowsPerPage, data.length - page * rowsPerPage);
    
        // Apply search and category filters to the data
        let filteredData = data.filter(item =>
            item?.institution?.ukprn.includes(searchQuery.toLowerCase())
            // && (selectedCategory === '' || item.session === selectedCategory)
        );
                        ...
                        <Table className={classes.table}>
                            ...
                            <TableBody>
                                {filteredData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map(n => {
                                    return (
                                        <TableRow key={n.institution.ukprn}>
                                            <TableCell component="th" scope="row">
                                                {n.institution.ukprn}
                                            </TableCell>
                                            <TableCell>{n.institution.location}</TableCell>
                                            {/* <TableCell>{n.duration}</TableCell>
                                            <TableCell>{n.variant}</TableCell>
                                            <TableCell>
                                                {n.progress}
                                                <CircularProgress variant="determinate" value={getPercent(n.progress)} size={20}/>
                                            </TableCell> */}
                                        </TableRow>
                                    );
                                })}
